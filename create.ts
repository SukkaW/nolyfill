...
/** top-level "overrides:" key and its indented entries */
const rPnpmWorkspaceOverrides = /^overrides:\n(?:[\t ].*\n)*/m;
...
(async () => {
  ...
  // pnpm reads its settings from pnpm-workspace.yaml, only regenerate the "overrides" block there
  const pnpmWorkspaceOverrides = `overrides:\n${allPackagesList.map(packageName => `  ${packageName}: workspace:@nolyfill/${packageName}@*\n`).join('')}`;
  const currentPnpmWorkspaceYaml = await fsPromises.readFile(pnpmWorkspaceYamlPath, 'utf-8');
  const newPnpmWorkspaceYaml = rPnpmWorkspaceOverrides.test(currentPnpmWorkspaceYaml)
    ? currentPnpmWorkspaceYaml.replace(rPnpmWorkspaceOverrides, () => pnpmWorkspaceOverrides)
    : currentPnpmWorkspaceYaml + pnpmWorkspaceOverrides;
...
export const allPackages = ${JSON.stringify(allPackagesList, null, 2)};\n`;
  ...
  await Promise.all([
    ...
    compareAndWriteFile(pnpmWorkspaceYamlPath, newPnpmWorkspaceYaml),
    compareAndWriteFile(
...
function generateDownloadStats() {
  ...
  const pkgList = [...nonNolyfillPackagesList, ...nolyfillPkgList];
